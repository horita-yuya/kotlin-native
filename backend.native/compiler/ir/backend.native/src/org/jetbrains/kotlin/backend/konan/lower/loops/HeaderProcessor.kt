...
internal sealed class ForLoopHeader(
    ...
    abstract fun comparingFunction(builtIns: IrBuiltIns): IrSimpleFunctionSymbol

    abstract fun initializeLoopVariable(symbols: KonanSymbols, builder: DeclarationIrBuilder): IrExpression

    abstract fun getDeclarations(): List<IrStatement>
}
...
internal class ProgressionLoopHeader(
        ...
        last: IrVariable,
        step: IrVariable
) : ForLoopHeader(inductionVariable, bound, last, step, headerInfo.closed, headerInfo.progressionType, null) {

    private val increasing = headerInfo.increasing

    override fun comparingFunction(builtIns: IrBuiltIns) = if (increasing)
    ...
    override fun getDeclarations(): List<IrStatement> = listOf(inductionVariable, step, bound, last)
}
...
internal class ArrayLoopHeader(
        ...
        last: IrVariable,
        step: IrVariable
) : ForLoopHeader(inductionVariable, bound, last, step, true, ProgressionType.INT_PROGRESSION, inductionVariable) {

    private val arrayDeclaration = headerInfo.arrayDeclaration
    private val isNewArrayDeclaration = headerInfo.isNewArrayDeclaration

    override fun comparingFunction(builtIns: IrBuiltIns) =
            builtIns.lessFunByOperandType[builtIns.int]?.symbol!!
    ...
    override fun getDeclarations(): List<IrStatement> =
            (if (isNewArrayDeclaration) listOf(arrayDeclaration) else emptyList()) +
                    listOf(inductionVariable, step, bound, last)
}
...
internal class HeaderProcessor(
    ...
    fun processHeader(variable: IrVariable): ForLoopHeader? {
        ...
        val progressionInfo = headerInfoBuilder.build(variable)
                ?: return null
        if (progressionInfo is ArrayHeaderInfo && progressionInfo.isNewArrayDeclaration) {
        ...
        with(builder) {
            with(progressionInfo) {
                ...
                // In case of `step` we need calculate the last element
